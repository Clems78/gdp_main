...
void set_global_position_target(double latitude, double longitude, double altitude, ros::NodeHandle& gnc_node) {
    ros::Publisher global_lla_pos_pub = gnc_node.advertise<mavros_msgs::GlobalPositionTarget>("/mavros/setpoint_raw/global", 10);
    ...
    global_lla_pos_pub.publish(target);
}
...
int main(int argc, char **argv) {
   //initialize ros 
    ...
    while(ros::ok() && current_waypoint_index < waypoints.size()) {
        ...
        set_global_position_target(target.latitude, target.longitude, target.altitude, gnc_node);
